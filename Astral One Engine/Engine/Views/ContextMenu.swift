...
public class ContextMenu {
    ...
    private let menuItem3Node = SKSpriteNode(imageNamed: "context-menu-item-1")
    private let menuItem4Node = SKSpriteNode(imageNamed: "context-menu-item-1")
    private let menuItem5Node = SKSpriteNode(imageNamed: "context-menu-item-1")
    private let menuItem6Node = SKSpriteNode(imageNamed: "context-menu-item-1")
    private let menuItem7Node = SKSpriteNode(imageNamed: "context-menu-item-1")
    private let menuItem8Node = SKSpriteNode(imageNamed: "context-menu-item-1")
    ...
    public init(game: Game, scene: SKScene, mapView: MapView) {
        ...
        menuItem2Node.position = CGPoint(x: 45, y: 60)
        menuItem3Node.position = CGPoint(x: 75, y: 0)
        menuItem4Node.position = CGPoint(x: 45, y: -60)
        menuItem5Node.position = CGPoint(x: 0, y: -75)
        menuItem6Node.position = CGPoint(x: -45, y: -60)
        menuItem7Node.position = CGPoint(x: -75, y: 0)
        menuItem8Node.position = CGPoint(x: -45, y: 60)
        ...
        self.game.$selectedMapPosition
            ...
            .sink(receiveValue: { mapPosition in
                ...
                let p = self.mapView.getCenterPoint(row: mapPosition.row, col: mapPosition.col)
                print("\(p)")
                self.menu.position = self.mapView.getCenterPoint(row: mapPosition.row, col: mapPosition.col)
