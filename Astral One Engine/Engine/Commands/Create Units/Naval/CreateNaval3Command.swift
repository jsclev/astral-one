...
public class CreateNaval3Command: Command {
    public private(set) var naval: Naval3
    ...
    public init(db: Db,
        ...
        naval = Naval3(player: player,
                              theme: Theme(id: Constants.noId, name: "Standard"),
                              name: "Naval3-\(Int.random(in: 0..<500))",
                              position: city.position)
        
        currentDisplayText = "Ready to create naval unit named \(naval.name)."
        currentDebugText = "Ready to create Naval3 unit named \(naval.name) (\(naval.id))."
