...
public class CreateNaval4Command: Command {
    public private(set) var naval: Naval4
    ...
    public init(db: Db,
        ...
        naval = Naval4(player: player,
                       theme: Theme(id: Constants.noId, name: "Standard"),
                       name: "Naval4-\(Int.random(in: 0..<500))",
                       position: city.position)
        
        currentDisplayText = "Ready to create naval unit named \(naval.name)."
        currentDebugText = "Ready to create Naval4 unit named \(naval.name) (\(naval.id))."
    ...
    public override func execute() -> CommandResult {
        ...
        let msg = "\(player.name) created infantry unit \(naval.name)"
