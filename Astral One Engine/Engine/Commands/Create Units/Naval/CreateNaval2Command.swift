...
public class CreateNaval2Command: Command {
    public private(set) var naval: Naval2
    ...
    public init(db: Db,
        ...
        naval = Naval2(player: player,
                       theme: Theme(id: Constants.noId, name: "Standard"),
                       name: "Naval2-\(Int.random(in: 0..<500))",
                       position: city.position)
        
        currentDisplayText = "Ready to create naval unit named \(naval.name)."
        currentDebugText = "Ready to create Naval2 unit named \(naval.name) (\(naval.id))."
